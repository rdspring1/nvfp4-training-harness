...
Paired with the pre-fix TorchAO arm
...
That TorchAO arm, its post-fix re-run, the TE-vs-TorchAO figure and the
root-cause analysis (`kernel_analysis/nvfp4_module_te_vs_ao.md`) are archived on
branch `nvfp4-te-audit`; the tables below carry their numbers.
...
See `training_loss_te_vs_ao_nvfp4_200m_tokens.png` on `nvfp4-te-audit` (lower
panel).
...
bf16 where TE reached 0.97. Root cause in `kernel_analysis/nvfp4_module_te_vs_ao.md`
on `nvfp4-te-audit`; fix in torchao `b3c77e59`.
