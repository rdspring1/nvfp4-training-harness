...
- Harness commit at launch: `08657ef` (Stage 1 result commit)
...
The run completed successfully with finite loss and emitted `Training completed`. Metrics were logged every 10 steps, so the final logged metric is step 760; successful completion of the configured 763-step loop is confirmed by the completion marker. The raw log retains recoverable CUDA allocator retry warnings observed near the memory ceiling.

| Final logged step | Loss | TPS / GPU | Aggregate TPS | TFLOPs / GPU | Peak Reserved Mem |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 760 | 1.27225 | 28,480 | 113,920 | 1,374.20 | 179.94 GiB |

Training runtime from `Training starts at step 1` to `Training completed` was 29m 37.2s. End-to-end runtime from the first torchrun log timestamp to process-group destruction was 29m 54.7s.
